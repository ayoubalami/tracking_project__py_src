...
class OfflineTracker:
    ...
    def start(self):
        ...
        video_name=(self.video_src.split("/")[-1]).split(".")[0]
        ...
        if self.tracking_service.detection_service and self.tracking_service.detection_service.get_selected_model() !=None:
            output_filename=video_name+"-"+self.tracking_service.detection_service.get_selected_model()['name']+"_"+str(self.threshold)+"^"+str(self.network_input_size)+"_"+str(self.tracking_service.use_cnn_feature_extraction)+"-track_out.mp4"
